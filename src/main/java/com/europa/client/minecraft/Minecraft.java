...
public class Minecraft extends net.minecraft.client.Minecraft implements IThreadListener, ISnooperInfo
{
    ...
    private void runGameLoop() throws IOException
    {
        ...
        for (int j = 0; j < Math.min(10, this.getTimer().elapsedTicks); ++j)
        {
            this.runTick();
        }
        ...
        this.soundHandler.setListener(this.getRenderViewEntity(), this.getTimer().renderPartialTicks); //Forge: MC-46445 Spectator mode particles and sounds computed from where you have been before
        this.profiler.endSection();
        ...
        {
            net.minecraftforge.fml.common.FMLCommonHandler.instance().onRenderTickStart(this.getTimer().renderPartialTicks);
            this.profiler.endStartSection("gameRenderer");
            this.entityRenderer.updateCameraAndRender(this.isGamePaused ? this.renderPartialTicksPaused : this.getTimer().renderPartialTicks, i);
            this.profiler.endStartSection("toasts");
            this.toastGui.drawToast(new ScaledResolution(this));
            this.profiler.endSection();
            net.minecraftforge.fml.common.FMLCommonHandler.instance().onRenderTickEnd(this.getTimer().renderPartialTicks);
        }
        ...
        GlStateManager.popMatrix();
        GlStateManager.pushMatrix();
        this.entityRenderer.renderStreamIndicator(this.getTimer().renderPartialTicks);
        GlStateManager.popMatrix();
        this.profiler.startSection("root");
        this.updateDisplay();
        Thread.yield();
        ...
        if (this.isGamePaused != flag)
        {
            ...
            {
                this.renderPartialTicksPaused = this.getTimer().renderPartialTicks;
            }
            else
            {
                this.getTimer().renderPartialTicks = this.renderPartialTicksPaused;
            }
    ...
    {
        return this.getTimer().renderPartialTicks;
    }

    public float getTickLength()
    {
        return this.getTimer().elapsedPartialTicks;
    }
